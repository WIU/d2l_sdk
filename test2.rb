...
get_key_api = '/d2l/auth/api/token'
httpmethod = 'GET'
...
# Pass these generated signature-bytes in all the query parameters where you’re
...
app_hash =  Base64.encode64(hash)
app_bytes = Base64URL.encode(app_hash)
ap app_bytes

hash = OpenSSL::HMAC.digest('sha256', user_key.encode('UTF-8'), app_str)
user_hash = Base64.encode64(hash)
user_bytes = Base64URL.encode(user_hash)
ap user_bytes
version_uri = "https://#{hostname}#{the_uri}?x_a=#{app_id}&x_b=#{user_id}&x_c=#{app_bytes}&x_d=#{user_bytes}&x_t=#{unix_ts}"
puts version_uri

RestClient.get(version_uri){ |response, request, result, &block|
  case response.code
  when 200
    p 'It worked !'
    ap response
